...
from modom_pypsa.buses import export_buses, normalize_bus_code
...
def test_export_buses(tmp_path: Path) -> None:
    ...
    assert len(payload["buses"]) == 3
    summary = payload["summary"]["counts"]
    assert summary["mapping_bus_count"] == 2
    assert summary["e_datred_bus_count"] == 3
    ...
    assert summary_json["counts"]["mapping_and_e_datred_overlap_count"] == 2
